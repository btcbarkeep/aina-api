...
class ContractorBuildingAccessCreate(BaseModel):
    building_id: str


class ContractorUnitAccessCreate(BaseModel):
    unit_id: str
...
@router.post(
    "/contractors/{contractor_id}/buildings",
    summary="Grant building access to a contractor",
    description="[Organization Access] Grant building access to a contractor. All users with contractor_id matching this contractor will inherit this access.",
    dependencies=[Depends(requires_permission("user_access:write"))],
)
def add_contractor_building_access(
    contractor_id: str,
    payload: ContractorBuildingAccessCreate
):
    """
    Grant building access to a contractor.
    All users with contractor_id matching this contractor will inherit this access.
    """
    client = get_supabase_client()
    ...
    if existing.data:
        raise HTTPException(400, "Contractor already has access to this building")
    ...
    try:
        ...
        logger.info(f"Granted building {payload.building_id} access to contractor {contractor_id}")
        return result.data[0]
    except Exception as e:
        raise HTTPException(500, f"Failed to grant building access: {e}")
...
@router.get(
    "/contractors/{contractor_id}/buildings",
    summary="List building access for a contractor",
    description="[Organization Access] List all buildings a contractor has access to",
    dependencies=[Depends(requires_permission("user_access:read"))],
)
def list_contractor_building_access(contractor_id: str):
    """List all buildings a contractor has access to."""
    client = get_supabase_client()
...
@router.delete(
    "/contractors/{contractor_id}/buildings/{building_id}",
    summary="Remove building access from a contractor",
    description="[Organization Access] Remove building access from a contractor",
    dependencies=[Depends(requires_permission("user_access:write"))],
)
def delete_contractor_building_access(contractor_id: str, building_id: str):
    """Remove building access from a contractor."""
    client = get_supabase_client()
    ...
    try:
        ...
        logger.info(f"Removed building {building_id} access from contractor {contractor_id}")
        return {
            "status": "deleted",
            "contractor_id": contractor_id,
            "building_id": building_id
...
@router.post(
    "/contractors/{contractor_id}/units",
    summary="Grant unit access to a contractor",
    description="[Organization Access] Grant unit access to a contractor. All users with contractor_id matching this contractor will inherit this access.",
    dependencies=[Depends(requires_permission("user_access:write"))],
)
def add_contractor_unit_access(
    contractor_id: str,
    payload: ContractorUnitAccessCreate
):
    """
    Grant unit access to a contractor.
    All users with contractor_id matching this contractor will inherit this access.
    """
    client = get_supabase_client()
    ...
    if existing.data:
        raise HTTPException(400, "Contractor already has access to this unit")
    ...
    try:
        ...
        logger.info(f"Granted unit {payload.unit_id} access to contractor {contractor_id}")
        return result.data[0]
    except Exception as e:
        raise HTTPException(500, f"Failed to grant unit access: {e}")
...
@router.get(
    "/contractors/{contractor_id}/units",
    summary="List unit access for a contractor",
    description="[Organization Access] List all units a contractor has access to",
    dependencies=[Depends(requires_permission("user_access:read"))],
)
def list_contractor_unit_access(contractor_id: str):
    """List all units a contractor has access to."""
    client = get_supabase_client()
...
@router.delete(
    "/contractors/{contractor_id}/units/{unit_id}",
    summary="Remove unit access from a contractor",
    description="[Organization Access] Remove unit access from a contractor",
    dependencies=[Depends(requires_permission("user_access:write"))],
)
def delete_contractor_unit_access(contractor_id: str, unit_id: str):
    """Remove unit access from a contractor."""
    client = get_supabase_client()
    ...
    try:
        ...
        logger.info(f"Removed unit {unit_id} access from contractor {contractor_id}")
        return {
            "status": "deleted",
            "contractor_id": contractor_id,
            "unit_id": unit_id
